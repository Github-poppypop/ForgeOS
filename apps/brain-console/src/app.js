  ...
  const renderBoard = () => {
    ...
    ["todo","in-progress","review","done"].forEach(status => {
      ...
      col.innerHTML = list.map(i => {
        ...
        const rightBtn = status !== "done" ? `<button class="btn secondary" data-move="${DOMPurify.sanitize(i.id)}" data-to="${status==="todo"?"in-progress":status==="in-progress"?"review":"done"}">→</button>` : "";
