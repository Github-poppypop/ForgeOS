...
// Mirrors the established client-test convention (see feat-agentcost.test.tsx):
...
describe('feat-abrouting.tsx', () => {
  it('default-exports a valid FeatureModule', async () => {
    const mod = await import('../feat-abrouting');
    ...
    assert.equal(fm.path, '/feature/ab-routing');
    assert.equal(typeof fm.label, 'string');
    assert.equal(fm.label, 'Agent A/B Canary Routing');
