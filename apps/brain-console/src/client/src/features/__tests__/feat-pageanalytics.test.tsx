...
// timer, or global listener, so this stays hermetic and never hangs.
describe('feat-pageanalytics.tsx', () => {
  it('default-exports a valid FeatureModule', async () => {
    const mod = await import('../feat-pageanalytics');
    ...
    assert.equal(fm.path, '/feature/page-analytics');
    assert.equal(typeof fm.label, 'string');
    assert.equal(fm.label, 'Page Analytics');
