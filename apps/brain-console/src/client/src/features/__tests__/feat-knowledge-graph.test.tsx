...
// Mirrors the established client-test convention (see feat-abrouting.test.tsx):
...
describe("feat-knowledge-graph.tsx", () => {
  it("default-exports a valid FeatureModule", async () => {
    const mod = await import("../feat-knowledge-graph");
    ...
    assert.equal(fm.path, "/feature/knowledge-graph");
    assert.equal(typeof fm.label, "string");
    assert.equal(fm.label, "Knowledge Graph");
    assert.equal(typeof fm.category, "string");
    assert.equal(fm.category, "Features");
