...
export const api = {
  ...
  compliance: () => req("/api/compliance"),
  ledgerSearch: (q) => req("/api/ledger/search?q=" + encodeURIComponent(q)),
  agentHeartbeat: () => req("/api/agent/heartbeat"),
  armDeadman: () => req("/api/agent/deadman", { method: "POST" }),
  memoryPool: () => req("/api/agent/memory"),
  amendments: () => req("/api/amendments"),
  sacred: () => req("/api/sacred"),
  processes: () => req("/api/processes"),
  createProcess: (data) => req("/api/processes", { method: "POST", body: JSON.stringify(data) }),
  portConflicts: () => req("/api/port-conflicts"),
  bustSW: () => req("/api/sw/cache-bust", { method: "POST" }),
  pluginManifest: () => req("/api/plugin-manifest"),
  replayOfflineQueue,
};
