...
# Clean launcher — sane PATH, correct cwd
...
# ForgeOS Phase 1 briefs (10 concerns)
cat > agents/briefs/brief-1.md << 'EOF'
You are ForgeOS farm agent-1. Work ONLY in /opt/forgeos/apps/brain-console. Do NOT modify C:\Projects\ForgeOS\governance.
...
cat > agents/briefs/brief-2.md << 'EOF'
...
cat > agents/briefs/brief-3.md << 'EOF'
...
cat > agents/briefs/brief-4.md << 'EOF'
...
cat > agents/briefs/brief-5.md << 'EOF'
You are ForgeOS farm agent-5. Work ONLY in /opt/forgeos/apps/brain-console/server.ts. Do NOT modify governance/.
...
cat > agents/briefs/brief-6.md << 'EOF'
You are ForgeOS farm agent-6. Work ONLY in /opt/forgeos/apps/brain-console/server.ts. Do NOT modify governance/.
...
cat > agents/briefs/brief-7.md << 'EOF'
...
Steps:
1) Create public/favicon.ico (minimal valid .ico; use Python: python3 -c "from PIL import Image; Image.new('RGBA',(16,16),(0,0,0,0)).save('public/favicon.ico')" — or base64-decode a tiny .ico).
...
cat > agents/briefs/brief-8.md << 'EOF'
...
cat > agents/briefs/brief-9.md << 'EOF'
...
cat > agents/briefs/brief-10.md << 'EOF'
You are ForgeOS farm agent-10. Work ONLY in /opt/forgeos. Do NOT modify governance/.
...
# Kill old, launch 10 tmux sessions
pkill -f '/opt/forgeos/agents/run-agent' 2>/dev/null || true
sleep 1
...
echo "launched: $(tmux list-sessions 2>/dev/null | wc -l) sessions"
tmux list-sessions 2>/dev/null || echo "tmux list-sessions failed"
echo "hermes procs: $(ps aux | grep hermes | grep -v grep | wc -l)"
