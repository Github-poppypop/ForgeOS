...
# ---------------------------------------------------------------------------
# 2. Handle existing runner installation
# ---------------------------------------------------------------------------
Write-Step "Checking for existing runner at $InstallPath"

$existingRunnerConfig = Join-Path $InstallPath ".runner"
$hasExistingConfig = Test-Path $existingRunnerConfig

if ($hasExistingConfig) {
    $currentUrl = ""
    $currentName = ""
    if (Test-Path $existingRunnerConfig) {
        $configLines = Get-Content $existingRunnerConfig -ErrorAction SilentlyContinue
        foreach ($line in $configLines) {
            if ($line -match '^url:\s*(.+)$') { $currentUrl = $matches[1].Trim() }
            if ($line -match '^name:\s*(.+)$') { $currentName = $matches[1].Trim() }
        }
    }

    if ($currentUrl -eq $repoUrl -and $currentName -eq $Name -and -not $Replace) {
        Write-Host "  Runner already configured for '$Name' on $repoUrl." -ForegroundColor Yellow
        Write-Host "  Use -Replace to reconfigure, or skip straight to service install."
    } elseif ($Replace) {
        Write-Host "  Existing runner found — replacing configuration (-Replace)."
    } else {
        Write-Warning "Runner at '$InstallPath' is configured for a different repo/name."
        Write-Warning "  Current: $currentName on $currentUrl"
        Write-Warning "  Requested: $Name on $repoUrl"
        if (-not $Replace) {
            throw "Add -Replace to overwrite the existing runner configuration, or choose a different -InstallPath / -Name."
        }
    }
} else {
    Write-Host "  No existing configuration found."
}

# If path exists but no config, or we're replacing, (re)download to get a clean binary set.
$needsDownload = -not $hasExistingConfig -or $Replace
...
if ($needsDownload) {
    ...
    Write-Host "  Skipping download — using existing binaries."
}
